...
import sys
import subprocess
from high_fidelity_fleet import SGPSMasterSolver
from relativity_math import schwarzchild_metric_function

def ensure_tmux():
    """Ensures the script runs inside a tmux session."""
    if "TMUX" not in os.environ:
        session_name = "SGPS_Phase1"
        print(f"Restarting phase1_sim.py in tmux session: {session_name}")
        subprocess.run(["tmux", "new-session", "-d", "-s", session_name, f"python3 {os.path.abspath(__file__)}"])
        print(f"Use 'tmux attach -t {session_name}' to view.")
        sys.exit(0)
